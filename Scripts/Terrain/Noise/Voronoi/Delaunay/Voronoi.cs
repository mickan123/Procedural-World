...
namespace csDelaunay
{
    public class Voronoi
    {
        ...
        public Site ClosestSiteAtPoint(Vector2 point)
        {
            float minDist = float.MaxValue;
            int siteIndex = -1;
            for (int i = 0; i < sites.sites.Count; i++)
            {
                float dist = Vector2.Distance(sites.sites[i].Coord, point);
                if (dist < minDist)
                {
                    minDist = dist;
                    siteIndex = i;
            ...
            return sites.sites[siteIndex];
        }

        public Site SecondClosestSiteAtPoint(Vector2 point)
        {
            float minDist = float.MaxValue;
            float secondMinDist = float.MaxValue;
            int siteIndex = 0;
            int secondSiteIndex = -1;
            for (int i = 0; i < sites.sites.Count; i++)
            {
                float dist = Vector2.Distance(sites.sites[i].Coord, point);
                if (dist < minDist)
                {
                    ...
                    minDist = dist;
                    secondSiteIndex = siteIndex;
                    siteIndex = i;
                }
                if (dist > minDist && dist < secondMinDist)
                {
                    secondMinDist = dist;
                    secondSiteIndex = i;
                }
            }
            return sites.sites[secondSiteIndex];
        }
        ...
        public List<Vector2> GetRegionAtPoint(Vector2 point)
        {
            List<List<Vector2>> regions = Regions();
            List<Vector2> region = null;
            for (int i = 0; i < regions.Count; i++)
            {
                if (ContainsPoint(regions[i], point))
                {
                    region = regions[i];
                }
            }
