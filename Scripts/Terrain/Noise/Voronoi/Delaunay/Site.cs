...
namespace csDelaunay {
	...
	public class Site : ICoord {
		...
		public Edge ClosestEdgeAtPoint(Vector2 point) {
			float minDist = float.MaxValue;
			Edge closestEdge = null;
			for (int i = 0; i < Edges.Count; i++) {
				Edge curEdge = Edges[i];
				if (curEdge.ClippedEnds != null) {
					
					float dist = Common.DistanceFromLine(
						point, 
						curEdge.ClippedEnds[LR.LEFT], 
						(curEdge.ClippedEnds[LR.LEFT] - curEdge.ClippedEnds[LR.RIGHT])
					);
		...
		public List<Vector2> GetVertices() {
			...
			for (int i = 0; i < Edges.Count; i++) {
				if (Edges[i].LeftVertex != null && !vertices.Contains(Edges[i].LeftVertex.Coord)) {
					vertices.Add(Edges[i].LeftVertex.Coord);
				}	
				if (Edges[i].RightVertex != null && !vertices.Contains(Edges[i].RightVertex.Coord)) {
					vertices.Add(Edges[i].RightVertex.Coord);
				}
			}
			return vertices;
		}
