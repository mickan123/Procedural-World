...
public static class ObjectGenerator
{
    ...
    public static List<ObjectSpawner> GenerateBiomeObjects(float[,] heightMap, BiomeInfo info, Road road, TerrainSettings settings, Vector2 sampleCentre)
    {
        List<ObjectSpawner> biomeObjects = new List<ObjectSpawner>();
        ...
        for (int biome = 0; biome < settings.biomeSettings.Count; biome++)
        {
            if (HeightMapContainesBiome(info, biome))
            {
                for (int objectSetting = 0; objectSetting < settings.biomeSettings[biome].terrainObjectSettings.Count; objectSetting++)
                {
                    ...
                    {
                        biomeObjects.Add(GenerateTerrainObject(
                            settings.biomeSettings[biome].terrainObjectSettings[objectSetting],
                            biome,
                            heightMap,
    ...
    public static ObjectSpawner GenerateTerrainObject(
    ...
    )
    {
        ...
        for (int point = 0; point < points.Count; point++)
        {
            spawnPositions.Add(
                new ObjectPosition(
                    points[point] + settings.GetTranslation(prng),
    ...
    public static void FilterSpawnPoints(
    ...
    )
    {
        ...
        {
            FilterPointsOnRoad(ref points, roadStrengthMap, prng);
        }
    }
